...
def _create_header():
    ...
    with gr.Row(variant="default"):
        with gr.Column(scale=4, min_width="300px"):
            ...
            text_powered_by = f'<span style="{text_style}">Powered by the Inverse Constitutional AI (ICAI) pipeline</span>'
            text_version = f'<span style="{text_style}">v{VERSION}</span>'
            link_github = f'<a href="https://github.com/ff-anon/feedback-forensics" style="{link_style}">🌟&nbsp;Star&nbsp;on&nbsp;GitHub</a>'
            link_report_bug = f'<a href="https://github.com/ff-anon/feedback-forensics/issues/new?template=Blank+issue" style="{link_style}">✍️&nbsp;Report&nbsp;bug</a>'
            link_get_in_touch = f'<a href="mailto:anonymous@example.com" style="{link_style}">📮&nbsp;Get&nbsp;in&nbsp;touch</a>'
            gr.HTML(
                image
...
def _create_numerical_results_panel(inp: dict, out: dict):
    ...
    with inp["numerical_results_col"]:
        ...
        gr.Markdown(
            "### Overall statistics\nSee (guide not available in anonymous submission) for metric details"
        )
        ...
        gr.Markdown(
            "---\n### Annotation metrics\n👉 *Click on values to view example datapoints* | See (guide not available in anonymous submission) to learn how each metric is computed and can be interpreted"
        )
